define.oldSkin({
    name: "wysiwyg.viewer.skins.gallery.SlideShowTextOverlay",
    Class:{
        Extends:"mobile.core.skins.BaseSkin",
        compParts:{
            imageItem: { skin:'wysiwyg.viewer.skins.displayers.SlideTextOverlayDisplayer', styleGroup:'inherit' }
        },
        _params:[
        ...
        _html:
                ...
                '<div class="buttons">' +
                    '<div class="btn" skinPart="buttonPrev"></div>' +
                    '<div class="btn" skinPart="buttonNext"></div>' +
                '</div>'+
        ...
        _css: [
                ...
                '%itemsContainer% { [pos][rd]overflow:hidden;' +
                    '-o-transform: translate3d(0,0,0); -webkit-transform: translate3d(0,0,0);' + // prevent flickering in Mac Safari
                '}',
//                '%itemsContainer% * { -o-transform: translate3d(0,0,0); -webkit-transform: translate3d(0,0,0);}',
            ...
            /*
                    '%autoplay% { display:inline-block; }',

                        '[state~=autoplayOff] %autoplay% { border:5px solid transparent; border-left: 5px solid [txt];  }',
                        '[state~=autoplayOn] %autoplay%  { border-left:2px solid [txt];  border-right:2px solid [txt]; margin-right:5px; height:10px; width:1px; }',  */
